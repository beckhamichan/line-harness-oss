...
このファイルは Claude Code 向けの指示です。実装AI（Codex）向けの `AGENTS.md` と**矛盾しない**よう運用します。両方読む場合は、共通方針は `AGENTS.md`、役割分担と開発ルールは本ファイルを正とします。
...
- **配信禁止時間帯: JST 23:00〜翌 7:00 は配信しない。**
...
1. 配信安全（特に 23:00〜7:00 禁止）と秘密情報保護
2. データを壊さない・二重送信しない
3. 小さくレビュー可能な単位にする
4. ゴールに対して前進する提案をする
