...
// Fake D1: handles the metadata SELECT/UPDATE + messages_log INSERT used by route-select.
function makeDb(metadata: string) {
  const updates: Array<{ sql: string; args: unknown[] }> = [];
  const db = {
    prepare: (sql: string) => ({
      bind: (...args: unknown[]) => ({
        first: async () => (sql.includes('SELECT metadata') ? { metadata } : null),
        run: async () => {
